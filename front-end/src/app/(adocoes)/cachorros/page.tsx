...
export default function Page() {
  ...
  const [CachorrosFiltrados, setCachorrosFiltrados] = useState<Animal[]>([]);
  const [porte, setPorte] = useState<string>();
  const [sexo, setSexo] = useState<string>();
  const [Cachorros, setCachorros] = useState<Animal[]>([]);
  ...
  useEffect(() => {
    api.get("/animais/disponiveis/?include=raca,imagens").then((response) => {
      ...
      const idEspecie = '6706d01b4893faa8e07ba536'
      const animaisFiltrados = animais.filter((animal: Animal) => idEspecie.includes(animal.raca?.especie_id))
      setCachorros(animaisFiltrados);
      setCachorrosFiltrados(animaisFiltrados);
    });
  }, []);
  ...
  const setFiltrados = ({ porteAtual, sexoAtual }: typeFiltrados) => {
    ...
    setCachorrosFiltrados(filtrados);
  };
    ...
    <div className="flex flex-col min-h-screen">
      <main className="flex-grow">
        ...
        <div className="p-6 flex flex-col items-center justify-center gap-6 overflow-x-hidden">
          <CarouselBanner />
          <h2 className="text-deep-blue font-semibold text-4xl">Cachorros</h2>
          {/**FILTROS  */}
          ...
          {CachorrosFiltrados.length === 0 && Cachorros.length > 0 && (
          ...
          <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-3 lg:w-5/6 2xl:grid-cols-4 max-w-screen mx-auto justify-items-center ">
            ...
            {CachorrosFiltrados.length > 1 &&
              CachorrosFiltrados.map((animalRender: Animal) => (
